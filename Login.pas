...
uses
  ...
  Vcl.ExtCtrls,   Vcl.StdCtrls, Vcl.Buttons,System.IniFiles,System.Win.Registry;
...
uses
  modConexao, ConfigurarIni,Inicial;
...
begin
  ...
  dmConexao.conexao.Server:= 'localhost';
  dmConexao.conexao.Database:= conexao;
  dmConexao.conexao.Username:= usuario;
  dmConexao.conexao.Password:= IntToStr(senha);
  dmConexao.conexao.port:= 5432;
...
begin
  try
    frmConfigurarINI := TfrmConfigurarINI.Create(Self);
    frmConfigurarINI.ShowModal;
  finally
    FreeAndNil(frmConfigurarINI);
  end;
end;
