...
procedure TfrmConfigurarINI.btnRemoverConexaoClick(Sender: TObject);
    ...
    if cbSecoes.ItemIndex = -1 then
      Application.MessageBox('Não foi selecionada nenhuma conexão', 'Aviso',mb_OK)
    else
    begin
      parametro:= cbSecoes.Items[cbSecoes.ItemIndex];
      Arquivo:= TIniFile.Create(lerRegistro + 'erp.ini');
      Arquivo.EraseSection(parametro);
      Application.MessageBox(PChar('A seção '+parametro+' foi removida com sucesso!'),'Informação',MB_OK+MB_ICONINFORMATION);
      carregar;
      LimparEdit;
    end;
	end;

procedure TfrmConfigurarINI.FormCreate(Sender: TObject);
begin
  btnRemoverConexao.Visible:= False;
  cbSecoes.Visible:= False;
end;
...
procedure TfrmConfigurarINI.btnEditarClick(Sender: TObject);
    ...
    begin
      ...
      btnRemoverConexao.Visible:= True;
      carregar();
    end;
